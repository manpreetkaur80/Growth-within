...
export default function Goals() {
  ...
  useEffect(() => {
    api.get("/goals")
      .then(data => setGoals(Array.isArray(data) ? data : []))      //req sent to api.get  , backend backend verifies JWT , then all goals loaded from database , here array recieved , then setGoals(data)
      .catch(err => console.error("Goals load error:", err));       //is array used to check if no array returns , any other error occir, then it went [] bcz next step it uses map function which is only valid for array . if any unauthorized ccome , so site crash
  }, []);
